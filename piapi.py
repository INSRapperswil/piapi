...
The module implements the PIAPI class which helps interacting with the 
Cisco Prime Infrastructure REST API using simple methods that can 
either request data or request an action.

The Cisco Prime Infrastructure API is a REST API which exposes several 
resources that can be of 2 types:
    * Data resources: expose some data collected by the software 
      which can be retrieved (e.g: client summary).
    * Service resources: expose some services that can modify the 
      configuration of the software (e.g: modify/update an Access Point)

The REST API is applying request rate limiting to avoid server's 
overloading. To bypass this limitation, especially when requesting data 
resources, the PIAPI uses multithreading requests (grequests library) 
with an hold time between chunk of requests. Please check the 
documentation to knowns more about rate limiting.

Also note that the piapi module only works with the JSON structure 
exposed by the REST API. The module doesn't support the default XML 
structure.

Please check your Cisco Prime REST API available at 
http://{server-name}/webacs/api/v1/
"""
...
"""
Default hold time in second to wait between group of concurrent request 
to avoid rate timiting (check *Rate Limiting* of the API)
"""
DEFAULT_HOLD_TIME = 1
"""
...
class PIAPICountError(PIAPIError):
    """
    Error raised by the piapi module when no result can be found for an 
    API request.
    """
...
class PIAPIResourceNotFound(PIAPIError):
    """
    Error raised by the piapi module when a requested resource is not 
...
class bsuPrimeAPI():
    ...
    def __init__(self,host,username,password,verify=True, virtual_domain=None):
        ...
        # Caching is used for data resource with keys as checksum of 
        # resource's name+params from the request
        self.cache = {} 

        # Service resources holds all possible service resources with 
        # keys as service name and hold the HTTP method + full url to 
        # request the service.
        self._service_resources = {}
        # Data resources holds all possible data resources with key as 
        # service name and value as full url access.
    ...
    def _parse(self, response):
        """
        Parse a requests.Response object to check for potential errors 
        using the HTTP status code. Please check your Cisco Prime 
        Infrastructure REST API documentation for errors and return 
        code.
        ...
        if response.status_code == 200:
            ...
            raise PIAPIRequestError(
                "The servers are up, but overloaded with requests. Try again later (rate limiting)"
                )
        else:
            raise PIAPIRequestError(
    ...
    def _request_wrapper(self, queue, url, params, timeout):
        ...
        response = self.session.get(
            url, params=params, verify=self.verify, timeout=timeout)
        queue.put(response)

    @property
    def resources(self):
        """
        List of all available resources to be requested. This includes 
        actions and data resources.
        """
    ...
    def data_resources(self):
        """
        List of all available data resources, meaning resources that 
        return data.
        """
    ...
    def service_resources(self):
        """
        List of all available service resources, meaning resources that 
        modify the NMS.
        """
        ...
        for entry in response_json["queryResponse"]["operation"]:
            self._service_resources[entry["$"]] = {
                "method": entry["@httpMethod"], 
                "url": "%s/op/%s.json" % (self.base_url,entry["@path"])}
    ...
    def request_data(
        ...
        """
        Request a 'resource_name' resource from the REST API. The 
        request can be tuned with filtering, sorting options.

        Check the REST API documentation for available filters by 
        resource. To bypass rate limiting feature of the API you can 
        tune paging_size, concurrent_requests and hold_time parameters.

        'X' concurrent requests will be sent as chunk and we will wait 
        the hold time before sending the next chunk until all 
        resource_name have been retrieved.
        ...
        params : dict (optional)
            Additional parameters to be sent along the query for 
            filtering, sorting,... 
            (default : empty dict).
        check_cache : bool (optional)
            Whether or not to check the cache instead of performing a 
            call against the REST API.
        timeout : int (optional)
            Time to wait for a response from the REST API 
            (default : piapi.DEFAULT_REQUEST_TIMEOUT)
        paging_size : int (optional)
            Number of entries to include per page 
            (default : piapi.DEFAULT_PAGE_SIZE).
        concurrent_requests : int (optional)
            Number of parallel requests to make 
            (default : piapi.DEFAULT_CONCURRENT_REQUEST).
        hold : int (optional)
            Hold time in second to wait between chunk of concurrent 
            requests to avoid rate limiting 
            (default : piapi.DEFAULT_HOLD_TIME).
        ...
        # Check the cache to see if the couple (resource + parameters) 
        # already exists (using SHA256 hash of resource_name and params)
        #
        # hash_cache = hashlib.sha256(
        #      b"%s%s" % (resource_name, params)).hexdigest()
        #  if check_cache and hash_cache in self.cache:
        #     return self.cache[hash_cache]

        # Get total number of entries for the request
        response = self.session.get(
            self._data_resources[resource_name], params=params, timeout=timeout)
        self._parse(response)
        count_entry = int(response.json()["queryResponse"]["@count"])
        if count_entry <= 0:
            raise PIAPICountError(
                "No result found for the query %s with params %s" % (response.url, params))
        ...
        for first_result in range(0, count_entry, paging_size):
            ...
            params_copy.update(
                {".full": "true", ".firstResult": first_result, ".maxResults": paging_size})
            ...
            paging_requests.append(
                threading.Thread(
                    ...
                    args=(queue, self._data_resources[resource_name], params_copy, timeout)))

        # Create chunks from the previous list of requests to avoid rate 
        # limiting (we hold between each chunk)
        chunk_requests = [paging_requests[x:x+concurrent_requests] for x in range(
            0, len(paging_requests), concurrent_requests)]

        # Bulk query the chunk pages by waiting between each chunk to 
        # avoid rate limiting
        responses = []
    ...
    def request_service(self, resource_name, params=None, timeout=DEFAULT_REQUEST_TIMEOUT):
        ...
        params : dict (optional)
            JSON parameters to be sent along the resource_name request 
            (default : empty dict)
        timeout : int (optional)
            Time to wait for a response from the REST API 
            (default : piapi.DEFAULT_REQUEST_TIMEOUT)
        ...
        if resource_name not in self.service_resources:
            raise PIAPIResourceNotFound(
                "Service Resource '%s' not found in the API, check 'service_resources' property "
        ...
        # if the HTTP method is 'GET', use the params args of request, 
        # otherwise use data (POST, DELETE, PUT)
        if method == "GET":
            response = self.session.request(
                method, url, params=params, verify=self.verify, timeout=timeout)
            ...
            response = self.session.request(
                method, url, data=params, verify=self.verify, timeout=timeout)
    ...
    def request(
        ...
        """
        Generic request for either data or services resources. The 
        parameters correspond to the ones from *PIAPI.request_data* or 
        *PIAPI.request_action*.
        ...
        virtual_domain : str (optional)
            Name of the virtual domain to send the request for 
            (default : None).
        check_cache : bool (optional)
            Whether or not to check the cache instead of performing a 
            call against the REST API.
        ...
        if resource in self.data_resources:
            return self.request_data(
                resource, params, check_cache, timeout, paging_size, concurrent_requests, hold)
